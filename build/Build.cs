...
class Build : NukeBuild
{
	...
	Target ReverseProxyBuid => _ => _
		.Executes(() =>
		{
			DockerBuild(s => s
				...
				.SetFile("./Dockerfile")
				.SetTag("quasar/chats/reverseproxy")
				.SetPath("."));
		});
	...
	Target Deploy => _ => _
		...
		.Executes(() =>
		{
			...
			DockerTag(s => s
				.SetSourceImage("quasar/chats/reverseproxy:latest")
				.SetTargetImage($"{DockerHubUrl}/quasar/chats/reverseproxy:latest"));

			DockerTag(s => s
				.SetSourceImage("quasar/chats:latest")
				.SetTargetImage($"{DockerHubUrl}/quasar/chats:latest"));
			
			DockerPush(s => s
				.SetName($"{DockerHubUrl}/quasar/chats/reverseproxy:latest"));

			DockerPush(s => s
				.SetName($"{DockerHubUrl}/quasar/chats:latest"));
		});
